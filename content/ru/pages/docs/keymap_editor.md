...
В окне надо выбрать Behavior для этой клавиши. Bahavior - это сущность в ZMK, определяющая какое именно действие будет произведено при нажатии клавиши, например `&kp` (Key Press) просто отправляет в ОС нажатие клавиши. Каждый экземпляр `&kp` имеет параметр `Key Code`, в котором мы указываем какая конкретно клавиша будет нажата, на скриншоте выше в этом параметре указана клавиша "Q". Если мы хотим назначить вместо "Q", например, "Z", то надо кликнуть на поле `Key Code`.
...
В ZMK для переключения слоев есть следующие функции:
1) &mo
