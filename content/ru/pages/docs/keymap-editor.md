...
> При создании кастомного *Hold or Tap behavior*, в `bindings - HOLD` и `TAP` можно указать любые *Поведения*, даже макросы - это очень гибко, ведь можно выполнять любые действия, какие только в голову придут, например в документации ZMK есть вот такая идея: макрос, в котором при активации слоя одновременно менять цвет на клавиатуре на заданный в параметре, то есть, вероятно, после переезда K:03 и Imperial на ZMK можно будет самому выбрать цвета светодиода для слоев! А ведь можно скомбинировать это не только со слоями, а с чем угодно, даже с модификаторами!
