...
#### &tog
`&tog` - при нажатии переключает сосостяние слоя, то есть включает указанный слой, если он выключен, и наоборот, выключает его, если он включен. *(аналог TG() из Vial)*
...
### Перемещение слоёв
Иногда при настройке раскладки возникает необходимость перемещения целого слоя на другую позицию, в keymap-editor это реализовано очень удобо, достаточно просто перетащить слой на нужную позицию.
...
*Поведение* Mod/Tap позволяет назначить определённое *Поведение* на **короткое нажатие** (*TAP*) клавиши, и другое *Поведение* на **зажатие** (*HOLD*) клавиши. По-умолчанию **коротким нажатием** считается нажатие, длящееся менее 200 милисекунд, если зажали дольше, то регистрируется **зажатие**.  
> С помощью этого *Поведения* можно сделать аналог кей-кодов с действиями по *HOLD* вкладки Quantum виала (*LShift_T*, *LCtrl_T* и остальные модификаторы).

![image](/images/keymap-editor/mt.png)


Если настроить *Поведение* как на скриншоте выше, то при **коротком нажатии** клавиши напечатается буква *D*, а при **зажатии** клавиши будет зажат левый Shift.  


### `&sk`

При **нажатии** Sticky Key задерживает клавишу нажатой, и отпускает её после следующего нажатия любой другой клавиши. Удобно использовать с модификаторами, назначив `&sk LShift` можно не зажимать клавишу, чтобы напечатать заглавную букву, а просто один раз **нажать** её и сразу отпустить, следующая буква уже будет заглавной
> Аналог OSM из виала.
...
На вкладке Behaviors можно настроить свои кастомные *Поведения* и настроить стандартные. Начнем с настройки стандартных.
...
В открывшемся окне *Reconfigure `&sl`* видим ряд стандартных свойств, которые мы можем изменить. Давайте увеличим таймаут для `&sl`, чтобы слой оставался активным не 1 секунду, а, например 3, для этого в строке свойства `release-after-ms` кликаем кнопку **Override** и в появившемся поле вводим 3000 (единицы измерения в поле - милисекунды), и нажимаем **Okay**. 

![image](/images/keymap-editor/reconfigured.png)


В разделе Reconfigure появилась запись `&sl` с переопеределенными свойствами. 
Просмотреть свойства можно нажав кнопку **Edit**. Также можно полностью вернуть свойствам стандартные значения, нажав кнопку **Delete**.  
...
* `bindings - HOLD`: `&kp`. При зажатии клавиши будет зажиматься клавиша модификатор, например *Ctrl*, для этого подойдет обычный `&kp`
* `bindings - TAP`: `&kp`. Действие при кратковременном нажатии, для того чтобы указать например клавишу *D* также подойдет `&kp`
* `tapping-term-ms`: 200. Если клавиша будет нажата **менее чем 200 милисекунд**, то клавиатура зарегистрирует TAP и нажмётся клавиша, например *D*, если клавиша будет зажата **более чем 200 милисекунд**, то зарегистрируется HOLD и нажмётся указанный в параметрах модификатор, напирем *Ctrl*.  
* `flavor`: balanced. Есть несколько алгоритмов, позволяющих пользоваться HRM не завязываясь строго на тайминг из `tapping-term-ms`, *balanced* это как раз один из них, он подразумевает, что если HRM клавиша была нажата менее чем 200 милисекунд, но за время нажатия HRM клавиши была **нажата и отпущена** другая клавиша, то надо зарегистрировать действие HOLD.  
...
Заполним параметры:
    * В *Parameters - HOLD* надо указать кей-код желаемого модификатора, укажем, например, *Ctrl*.
    * В *Parameters - TAP* надо указать кей-код клавиши, отправляемой по тапу, например *D*.

![image](/images/keymap-editor/hrm3.png)  


> Интересной идеей может быть назначение HRM модификатора *Ctrl* на клавишу *S*, ведь тогда можно нажимать очень часто используемые сочетания *Ctrl + C* и *Ctrl + V* очень удобно одной рукой, попробуйте!

После заполнения параметров жмём **Okay** и наш первый *Home Row Mod* назначен!  
Аналогичным образом назначаем HRM модификаторы на все остальные клавиши, и, вуаля, все модификаторы у нас уже под пальцами, можно достать их совершенно не смещая руки.

![image](/images/keymap-editor/hrm4.png)  

Совет: Можно создать несколько *Поведений* с разным Tapping Term под разные пальцы, ведь мизинец явно слабее указательного, и нажимать клавишу будет дольше.  


# Полезные кастомные Behaviors

Для создания клавиш с перечисленным ниже поведением надо создать новое кастомное *Поведение* с соответствующим типом. 


### `&td`

С помощью Tap Dance можно назначить разное *Поведение* в зависимости от того, сколько раз подряд клавиша была нажата.  
...
> С помощью комбинации этого поведения и `&mt` можно сделать клавишу, аналогичную Tap Dance из виала.
...
Если назначить Mod Morph со скриншота на клавишу, то при нажатии без модификаторов напечатается буква *Q*, а при нажатии с зажатым модификатором *Alt* нажмётся *Esc*. Вместо *Esc* может быть любое поведение, например изменение слоя, или макрос!
> Это поведение можно считать более гибким аналогом Key Overrides из виала. 
...
Заполнеяем название комбо (на скриншоте "lang"), и выбираем *Поведение* при активации комбо, для изменения языка раскладки подойдет `&kp`, нажимающий *Space* с зажатым модификатором *GUI*.
...
# Макросы
С помощью *Макросов* можно сохранить какую-то часто используемую последовательность сочетаний клавиш и вызывать эту последовательность нажатием всего одной клавиши, например можно сделать *Макрос*, печатающий различные шаблоны, вроде такого:  
>\`\`\`
>
>\`\`\`  
...
Далее нам надо составить действия, которые выполнит *Макрос*. Чтобы макрос трижды напечатал символ **\`**, трижды используем *Поведение* `&kp`. При создании макроса ячейка под одно *Поведение* сразу добавляется, добавим еще 2, дважды нажав кнопку **Add Key Binding**, и во всех трёх ячейках назначим ``&kp \```.
...
По умолчанию ZMK будет нажимать каждую клавишу 30 мс, и между каждым нажатием будет происходить пауза 15 мс, чтобы ОС успевала обрабатывать каждое нажатие. Чтобы действия точно сработали (во всяких тормозных корпоративных мессенждерах, удалённых рабочих столах и джирах), а также чтобы совершаемые *Макросом*, были немного читаемее, разделим их на группы добавив дополнительную паузу в 10 мс, для этого нажмём **Add Control Binding** и перенесём появившийся `&macro_press` после первых трёх нажатий **\`**. Теперь надо не забыть поменять `&macro_press` на `&macro_wait_time`, для этого кликнем на него и выберем `&macro_wait_time`, в параметре *Raw Value* укажем 10, это количество милисекунд которое подождет макрос.
...
* `&macro_tap` - для последующих нажатий устанавливает режим, при котором указанная клавиша будет **нажата**, и по прошествии времени, указанном в *tap_time*, будет **отпущена**;
* `&macro_tap_time` - задаёт время между **нажатием** и **отпусканием** каждой клавиши в макросе;
* `&macro_pause_for_release` - на этом месте *Макрос* остановит своё выполнение до момента, когда клавиша *Макроса* будет **отпущена**.

Применив перечисленные *Control Bindings* можно составить например макрос, который при **нажатии** переключает слой, нажимает определенный модификатор, далее дожадается **отпускания** макроса, и после этого уже возвращает слой обратно и отпускает модификатор. Получается кнопка, объединяющая `&mo` и `&kp`, два в одном!
...
У *Макросов* в ZMK есть одна интересная особенность, их можно сделать параметризованными. Для использования параметров надо применить один или несколько из перечисленных *Control Bindings*:
...
Рассмотрим на примере, реализовав *Макрос*, который при **нажатии** активирует переданный в параметре слой, нажимает переданный в параметре модификатор, ждёт **отпускания** клавиши, а затем выключает переданный слой и отпускает переданный в параметря модификатор.  
Назовём *Макрос* `&ml` (mod-layer), он будет выглядеть вот так:

![image](/images/keymap-editor/macro7.png)  

* Выбраны `&macro_param_1to1` и `&macro_param_2to1` так как используемые *Поведения* `&mo` и `&kp` имеют по одному входному параметру.  

Тут стоит обратить внимание на то, как *Макрос* назначается в раскладке. В списке Behavior находим созданный *Макрос* `&ml`, выбрав его нам необходимо заполнить параметры: 
1) *Layer* - это слой, который передастся в качестве параметра для `&mo`, выберем 2;
2) *Key Code* - это клавиша, которая будет нажата одновременно с `&mo`, мы планировали использовать модификатор, так что выберем *LShift*.  
...
> Кстати, при создании кастомного *Hold or Tap behavior*, в `bindings - HOLD` и `TAP` можно указать любые *Поведения*, даже макросы - это очень гибко, ведь при **удержании** клавиши можно выполнять любые действия, какие только в голову придут, например в <a href="https://zmk.dev/docs/keymaps/behaviors/macros#layer-activation--more" target="_blank">документации ZMK</a> есть вот такая идея: параметризованный макрос, в котором при активации слоя одновременно меняется и цвет RGB-подсветки на заданный в параметре, то есть, вероятно, после переезда K:03 и Imperial на ZMK можно будет самому выбрать цвета светодиода для слоев! А ведь можно скомбинировать это не только со слоями, а с чем угодно, даже с модификаторами!  